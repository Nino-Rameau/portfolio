...
// const pour etat du mode sombre et voir sur quel page est l'utilisateur
const Header = () => {
  ...
  return (
    <header className="flex flex-row items-center justify-around bg-gradient-to-r from-bleu_clair2 to-bleu_milieu dark:bg-gradient-to-r dark:from-bleu_milieu dark:to-bleu_fonce h-20 sticky top-0 z-50">
      ...
      <nav className="flex space-x-10">
        <Navbtn to="/" active={active.pathname === "/"}>Accueil</Navbtn>
        <Navbtn to="/competences" active={active.pathname === "/competences"}>Compétences</Navbtn>
        <Navbtn to="/projets" active={active.pathname === "/projets"}>Projets</Navbtn>
        <Navbtn to="/charteGraphique" active={active.pathname === "/charteGraphique"}>Charte Graphique</Navbtn>
        <Navbtn to="/cv" active={active.pathname === "/cv"}>CV</Navbtn>
        <Navbtn to="/contact" active={active.pathname === "/contact"}>Contact</Navbtn>
      </nav>

      {/* gestion mode sombre / clair (change entre 2 svg) */}
      <button title="Mode clair / sombre" onClick={() => setDarkMode(!darkMode)}>
        {darkMode ? (
          <svg
